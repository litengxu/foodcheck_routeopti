...
import com.bjfu.fcro.logic.wjx.BasicOperation;
...
public class Two_Opt {
	...
	public static int[] Two_opt(int[] initPath,double [][]dists,int optcount) {
		...
		while(count<=optcount) {
			...
			initValue = com.bjfu.fcro.logic.wjx.BasicOperation.getDistFromPath(initPath, dists);
	...
	public static int[] Two_opt2(double curTime, int[] initPath, double [][]dists, int optcount, List<DistInDifTime> Dlist) {

		int n = initPath.length;
		double initValue = com.bjfu.fcro.algorithm.BasicOperation.getDistFormPathByDifTime2(curTime,initPath, dists,Dlist);
		double bestValue = initValue;
		//选取两个点(1-n-1)不交换0
		int count=1;
		while(count<=optcount) {
			...
			initValue = com.bjfu.fcro.algorithm.BasicOperation.getDistFormPathByDifTime2(curTime,initPath, dists,Dlist);
			if(initValue < bestValue) {
				bestValue = initValue;
				count=1;
