...
import com.bjfu.fcro.algorithm.Chromosome;
...
//		double c = 0.85; //温度衰减因子
		...
		bestpath = Path.clone();
		double bestvalue = com.bjfu.fcro.logic.wjx.BasicOperation.getDistFromPath(bestpath, dists);
...
//					temp = Random_Twopoint.ran_twopoint(temp); //随机调换两个点之间 的值
					temp = Kopt.kopt(temp,dists,200,2);
		...
		for(int i = 0;i<GenerationNum;i++) {
			...
			if(randomvalue > early) {//大于预警值说明安全
				...
				for(int j = 0;j<discovervalue;j++) {
					...
					double fitness1 = pool[j].fitness;
//					temp = Random_Twopoint.ran_twopoint(temp); //随机调换两个点之间 的值
					temp = Kopt.kopt2(temp,dists,200,2,true,Dlist);
					temp = PartitionSwap.partionswap2(curTime,temp,dists,200,1,Dlist);
//					temp = two_opt.Two_opt_sa(temp,dists,2);//进行一次2-opt 不管适应度是否进步
//					double fitness2 = BasicOperation.getDistFromPath(temp, dists);
//					temp = Simulated_Annealing.sa(pool[j].genes, fitness1, temp, fitness2, T);//模拟退火选择接受与否
...
//					}
					pool[j].genes = Arrays.copyOf(temp, n);
					pool[j].getFitness2(curTime,dists,Dlist);
...
//					temp = Random_Twopoint.ran_twopoint(temp); //随机调换两个点之间 的值
					...
					temp = Two_Opt.Two_opt2(curTime,temp,dists,200,Dlist);
					temp = PartitionSwap.partionswap2(curTime,temp,dists,200,2,Dlist);//分段交换，原则上加快收敛，局部优化，避免陷入局部最优。改变较少，不会破坏大的路径



//					temp = two_opt.Two_opt_sa(temp,dists,2);//进行一次2-opt 不管适应度是否进步
					double fitness2 = BasicOperation.getDistFormPathByDifTime2(curTime,bestpath, dists,Dlist);
//					temp = Simulated_Annealing.sa(pool[j].genes, fitness1, temp, fitness2, T);//模拟退火选择接受与否
