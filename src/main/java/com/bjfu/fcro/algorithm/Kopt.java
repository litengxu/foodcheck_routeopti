...
public class Kopt {
	...
	public static int[] kopt(int[] initPath,double [][]dists,int optcount,int k) {//分为k段，每段选两个数，并逆置中间的数，择优
		...
		double initValue = initValue = com.bjfu.fcro.algorithm.BasicOperation.getDistFromPath(initPath, dists);
		com.bjfu.fcro.algorithm.BasicOperation.getDistFromPath(initPath, dists);
		...
		while(count<=optcount) {
			...
			for(int i = 0;i<k;i++) {
				...
				initValue = com.bjfu.fcro.algorithm.BasicOperation.getDistFromPath(initPath, dists);
...
//			}
	...
	public static int[] kopt2(int[] initPath, double [][]dists, int optcount, int k, boolean time_sharing, List<DistInDifTime> Dlist) {//分为k段，每段选两个数，并逆置中间的数，择优
		int n = initPath.length;//长度
		int s = (n-1)/k;//分为k段,每段s.0点不参与
		double initValue = 0.0;
		if(time_sharing){
			initValue = BasicOperation.getDistFormPathByDifTime2(System.currentTimeMillis(),initPath,dists,Dlist);
		}else{
			initValue = com.bjfu.fcro.algorithm.BasicOperation.getDistFromPath(initPath, dists);
		}
//		com.bjfu.fcro.algorithm.BasicOperation.getDistFromPath(initPath, dists);
