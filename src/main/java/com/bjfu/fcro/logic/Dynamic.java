...
public class Dynamic {
	...
	private void initstoreMap() {
		...
		for (int i = 0; i < this.length; i++) {
			...
			for (int j = 0; j < this.length - 1; j++) {
				...
				store.put(this.toLengthOfLength(i) + this.toLengthOfLength(j),
						this.dArray[this.length - 2][i] + store.get(this.toLengthOfLength(j)));
			}
		}
	}
	...
	private void guihua(int temp) {
		...
		for (int i = 0; i < (list.size() - 1); i++) {
			int next = (i + 1);
			if (list.get(i).substring(0, temp * this.lengthOfLength)
					.equals(list.get(next).substring(0, temp * this.lengthOfLength))) {
				double iValue = 0;
				double nextValue = 0;
				...
				nextValue = this.dArray[temp][Integer
						.parseInt(list.get(next).substring(temp, temp + this.lengthOfLength))]
...
/*
	public static void main(String[] args) {
		double[][] first = { // 各个节点之间路径长度的二维数组
				{ 0, 2, 1, 3, 4, 5, 5, 6 }, { 1, 0, 4, 4, 2, 5, 5, 6 }, { 5, 4, 0, 2, 2, 6, 5, 6 },
				{ 5, 2, 2, 0, 3, 2, 5, 6 }, { 4, 2, 4, 2, 0, 3, 5, 6 }, { 4, 2, 4, 2, 3, 0, 5, 6 },
				{ 4, 2, 4, 2, 4, 3, 0, 6 }, { 4, 2, 4, 2, 8, 3, 5, 0 } };
		long start = System.currentTimeMillis();
		Dynamic ff = new Dynamic(first);
